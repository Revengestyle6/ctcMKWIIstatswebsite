...
| --- | --- | --- |
| `ctc-api-staging@mkw-stats.iam.gserviceaccount.com` | Staging Cloud Run API | Cloud SQL Client; object access on the staging archive bucket; accessor on the staging database URL and rate-limit HMAC secrets |
| `ctc-api-prod@mkw-stats.iam.gserviceaccount.com` | Production Cloud Run API | Cloud SQL Client; object access on the production archive bucket; accessor on the production database URL and rate-limit HMAC secrets |
...
Runtime bucket permissions use `roles/storage.objectUser` on one environment's
bucket, not on the project. Runtime secret access uses
