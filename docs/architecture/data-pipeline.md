...
owns the durable accepted canonical bytes:

```text
gs://{environment-archive}/accepted/{league}/{season}/{division}/{match}--{sha256-prefix}.json
```

These accepted bytes are the reviewed canonical match document, not necessarily
an unchanged copy of the originally submitted document. The review queue and
accepted archive have separate lifecycles.
...
[MKCentral Player Names](../features/mkcentral-player-names.md).
