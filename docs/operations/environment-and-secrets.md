...
| `LOG_LEVEL` | Backend | Python request/application log level; defaults to `INFO` |
| `GUNICORN_WORKERS` | `start.sh` | Worker processes; defaults to 1 |
| `GUNICORN_THREADS` | `start.sh` | Threads per worker; defaults to 8 |
| `GUNICORN_TIMEOUT_SECONDS` | `start.sh` | Worker timeout; defaults to 60 seconds |
| `CLOUD_SQL_INSTANCE` | Admin access runbook response | Displayed instance name for human onboarding; defaults to a placeholder |
| `CLOUD_SQL_CONNECTION_NAME` | Admin access runbook response | Displayed proxy connection name; does not configure the application connection |
| `GITHUB_REPOSITORY` | Admin access runbook response | Repository used in collaborator instructions; defaults to this project |
...
[local development startup runbook](../development/local-development-startup.md) for the
...
Cloud Run receives database and storage configuration through deployment
settings and Secret Manager. Administrator requests carry short-lived Firebase ID
tokens; the backend verifies them and checks `admin_users`. The staging GitHub Actions workflow uses
