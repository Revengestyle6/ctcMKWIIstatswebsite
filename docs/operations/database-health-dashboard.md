...
- Optional query: `include_archive=0` skips archive reconciliation, including
  accepted-object reads through the configured storage adapter.
- The handler builds a fresh report for each request; it has no response-cache
  decorator. Archive hashing and fuzzy catalog comparisons can make a full report
  more expensive than an ordinary analytics query.

The home page links to the dashboard.

Both report reads and review writes require an authorized application
administrator. Hosted requests use verified Firebase ID tokens; explicitly enabled
local/test authentication can use a development identity. A dismissal includes an
issue key, `dismissed` status, and required reason. Restoring a finding writes
`open` status. The separate public `/api/health/data` endpoint exposes only a
limited operational summary.
...
Scheduled `amcheck`/`pg_amcheck` maintenance and display of its last scan result
remain proposed work. They require an appropriately privileged maintenance
identity; the current dashboard does not launch a physical corruption scan.
...
- accepted source objects that cannot be read through the configured archive
  adapter, or legacy source files missing from the local archive root;
- stored content whose SHA-256 hash differs from `source_files`; and
- local JSON/text archive files that are not represented by a known path or hash
  in the database, ignoring a legacy `.txt` when its same-stem `.json` exists.

Accepted-object read failures, including storage errors, are reported in
`missing_files`; that label alone does not prove the object was deleted. Orphan
scanning traverses the local archive root, not the contents of the Cloud Storage
bucket.
...
| Archive mismatch | Compare the recorded SHA-256 with the accepted storage object, or the local source path for legacy files; check read permissions and storage availability too. | Repair the archive through the reviewed recovery workflow. It cannot be dismissed. |
...
This investigation records the checked-in historical source corpus, not a live
count of current staging findings.
...
Active decisions are stored in PostgreSQL `health_issue_reviews`, keyed by the
finding's stable issue key. Each decision records its reason, reviewer account,
and timestamp; the endpoint also writes an administrator audit event. They are
shared through the database and require database backup/recovery, not a Git commit.
An archive import does not itself reconstruct these decisions.

`backend/data/database_health_reviews.json` retains historical review evidence.
The file adapter remains available to explicitly selected local/legacy callers,
but application report requests pass a database session and do not automatically
seed or merge that JSON into PostgreSQL.

The historical file reviewed `GBA Bowser Castle 2` and `GBA Bowser Castle 4` as
distinct tracks. That finding appears under **Show dismissed** only if the active
database contains its dismissal. A different Bowser Castle typo receives a
different key and is still reported.

## Operational And Security Notes

- The dashboard only writes review metadata after an authenticated administrator
...
- The report and review endpoints already enforce administrator authentication.
  Detailed database metadata and player findings are not public endpoints.

## Potential Next Steps

These are proposals, not a description of deployed scheduled maintenance or new
health-page actions. Existing alias and identity editing tools live in Database
Management.

### 1. Expand Issue State

The `health_issue_reviews` table persists current review decisions. A future
...
This would support richer multi-user assignment and resolution history alongside
the existing database-backed review decisions.
...
Authentication and persistent reviews already exist. A possible health-page
extension would link findings to the existing audited Database Management
workflows, or add focused actions for:
...
Backend tests should seed small disposable PostgreSQL datasets and verify each
check independently, including clean control cases. Endpoint tests should verify
administrator authorization, report serialization, persistent reviews, and
archive-check options. Frontend validation should include the
