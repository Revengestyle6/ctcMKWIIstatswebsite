...
## Staging status and dated evidence

The repository targets Firebase/Google Cloud staging and includes a working
staging release workflow. Production cutover is a separate gate. The
[resource inventory](phase-4-resource-inventory.md) records dated provisioning and
acceptance observations; its match counts, schema revisions, and deployment IDs
are historical observations, not live status. This guide's checkpoint sections
below are also dated evidence. Reconcile the inventory before operational changes.

Monitoring/scheduled operations, recovery rehearsals, and the remaining staging
acceptance matrix are tracked in the [production gates](../roadmap/README.md).
...
Use the root [README](../../README.md). Flask listens on port 5000 and Vite on port 3000. The Vite
