...
Administrators manage team names and tags from **Database Management → Teams →
...
- The Identity tab edits existing participation entries. Database Management's
  Competition Setup workflow can register a team's first season/division entry
  before any match is imported; approved match import can also create an entry.
...
**Database Management → Teams** can review and merge a source team into a destination
