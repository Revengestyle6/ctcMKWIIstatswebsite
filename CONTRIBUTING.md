...
Start with [local setup](docs/development/local-development-startup.md), then read
[CONTEXT.md](CONTEXT.md) and the [repository map](docs/architecture/repository-map.md).
Issues and specifications live in GitHub Issues; use the
[issue workflow](docs/agents/issue-tracker.md) and [triage labels](docs/agents/triage-labels.md).

## Making a change

- Keep competition and identity terms consistent with the glossary. Put
  implementation explanations in architecture/feature guides, not the glossary.
- Keep a behavior's implementation and tests near its owning module. Preserve
  public routes and documented interfaces during structural refactors.
- Backend routes parse HTTP input and authorize callers; match policy belongs in
  the owning domain module. Callers own transactions unless an interface explicitly
  documents otherwise. Production schema changes use new Alembic revisions.
- Frontend route modules load lazily. Feature internals stay in their feature
  directory; shared presentation belongs in `components`, shared configuration in
  `config`, and HTTP contracts in the existing client modules.
- Ruff owns Python style. Biome and strict TypeScript own frontend style and type
  checks. Use the existing tools rather than adding competing formatters.
- Preserve historical input archives, reviewed registries, migrations, and approved
  baselines. Update source-linked documentation and Mermaid diagrams alongside a
  behavior or responsibility change.
- Separate current behavior, dated deployment evidence, and planned behavior.
  Describe the change and its verification in the PR; use a new ADR for a meaningful
  reversal of an accepted architecture decision.
