...
export function DashboardScopeControls({
  ...
  const seasons = Array.from(new Set(appearances.map((entry) => entry.season))).sort((a, b) =>
  ...
  const filteredEntities = entityOptions.filter(
    (option) =>
      (!season || !option.season || option.season === season) &&
      (!division || !option.division || option.division === division)
  );
  const uniqueEntities = Array.from(
    new Map(filteredEntities.map((option) => [option.id, option])).values()
  );
  const controlClass =
    "min-h-10 rounded-md border border-white/20 bg-zinc-950 px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-blue-400 disabled:opacity-50";
    ...
    <div className="flex flex-col gap-3 md:flex-row md:flex-wrap md:items-end">
      ...
      <label className="flex min-w-36 flex-col gap-1 text-sm font-semibold text-gray-300">
        ...
        <input
          ...
          disabled={disabled}
          onChange={(event) =>
            onMinRacesChange(Math.min(500, Math.max(1, Number(event.target.value) || 1)))
          }
        />
      </label>
    </div>
...
export function ResultBadge({ result }: { result: string }) {
  ...
  return (
    <span
      className={`inline-flex min-w-12 justify-center rounded border px-2 py-1 text-xs font-bold ${color}`}
    >
      {result.toUpperCase()}
    </span>
  );
  ...
  return (
    <div className="border-l-2 border-blue-400 pl-4">
      ...
      <p className="mt-1 text-2xl font-bold">
        {rank ? `#${rank} of ${population}` : "Not qualified"}
      </p>
...
export function TrendRows({
  ...
  if (values.length === 0)
    return <p className="text-sm text-gray-400">No trend data in this scope.</p>;
    ...
    <div className="space-y-2">
      {values.map((item) => {
        ...
        return (
          <div
            key={item.id}
            className="grid grid-cols-[5rem_1fr_3.5rem] items-center gap-3 text-sm"
          >
            <span className="truncate text-gray-400" title={item.label}>
              {item.label}
            </span>
            ...
            <span
              className={`text-right font-semibold ${unavailable ? "text-gray-500" : "text-gray-200"}`}
            >
