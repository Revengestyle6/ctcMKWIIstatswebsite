...
export function teamTag(teamKey: string, team: TeamJson): string {
  ...
  return String(team.table_tag_str ?? "")
...
function isMissingPlayerPlaceholder(player: MatchPlayerJson): boolean {
  return [player.table_str, player.mii_name, player.lounge_name, player.table_name].some((value) =>
    String(value ?? "")
      .toLowerCase()
      .includes("missing player")
  );
}

export function allPlayers(
  match: MatchJson
): Array<{ playerKey: string; friendCode: string; teamKey: string; player: MatchPlayerJson }> {
  return Object.entries(match.teams ?? {}).flatMap(([teamKey, team]) =>
    Object.entries(team.players ?? {}).map(([friendCode, player]) => ({
      playerKey: `${teamKey}::${friendCode}`,
      friendCode,
      teamKey,
      player,
    }))
  ...
  return Array.from({ length: count }, (_, raceIndex) => {
    ...
    const missingPlayerResults = Object.entries(match.teams ?? {}).flatMap(([teamKey, team]) => {
      const explicitResults = (team.missing_player_results ?? [])
        ...
        .map((result) => ({
          teamKey,
          score: result.score,
          reason: result.reason ?? ("unknown" as MissingPlayerReason),
        }));
    ...
    players.forEach(({ playerKey, teamKey, player }) => {
      ...
      if (typeof position === "number" && typeof score === "number")
        scorePairs.push({ position, score });
    });
    const largestPosition = Math.max(0, ...Array.from(byPosition.keys()));
    const candidates = Object.keys(SCORE_TABLES)
      .map(Number)
      .filter((size) => size >= largestPosition);
    const scoredCandidates = candidates
      .map((size) => ({
        size,
        matches: scorePairs.filter(
          ({ position, score }) => scoreForPosition(position, size) === score
        ).length,
      }))
      .sort(
        (left, right) =>
          right.matches - left.matches ||
          Math.abs(left.size - fallbackRoom) - Math.abs(right.size - fallbackRoom)
      );
    const roomSize = scoredCandidates[0]?.matches
      ? scoredCandidates[0].size
...
function gpGroups(scores: Array<number | null>): Array<Array<number | null>> {
  ...
  for (let index = 0; index < scores.length; index += 4)
    result.push(scores.slice(index, index + 4));
  return result;
}
...
export function compileMatch(match: MatchJson, races: RaceDraft[]): MatchJson {
  ...
  const teams = Object.fromEntries(
    Object.entries(normalizedMatch.teams ?? {}).map(([teamKey, team]) => {
      ...
      const players = Object.fromEntries(
        Object.entries(team.players ?? {}).map(([friendCode, player]) => {
          ...
          const scores = positions.map((position, index) => {
            ...
            return (
              orderedRaces[index].unplacedResults.find((result) => result.playerKey === key)
                ?.score ?? null
            );
          });
          const roles = orderedRaces.map(
            (race) =>
          ...
          const total =
            scores.reduce<number>((sum, score) => sum + (score ?? 0), 0) - (player.penalties ?? 0);
          const played = positions
            .map((position, index) => (position ? index : -1))
            .filter((index) => index >= 0);
          const gpScores = gpGroups(scores);
          const gpTotals = gpScores.map((gp) =>
            gp.reduce<number>((sum, score) => sum + (score ?? 0), 0)
          );
      ...
      const gross = Object.values(players).reduce<number>(
        (sum, player) => sum + Number((player as MatchPlayerJson).total_score ?? 0),
        0
      );
      const missingPlayerScores = orderedRaces.map((race) => {
        const scores = race.missingPlayerResults
          .filter((result) => result.teamKey === teamKey)
          .map((result) => result.score);
        return scores.length ? scores.reduce((sum, score) => sum + score, 0) : null;
      });
      const missingPlayerTotal = missingPlayerScores.reduce<number>(
        (sum, score) => sum + (score ?? 0),
        0
      );
      const missingPlayerResults = orderedRaces.flatMap((race) =>
      ...
      return [
        tag,
        {
          ...
          players,
          missing_player_scores: missingPlayerScores.some((score) => score !== null)
            ? missingPlayerScores
            : undefined,
