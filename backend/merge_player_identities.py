...
from database import DEFAULT_DB_PATH, BASE_DIR
...
def merge_alias(conn, keep_player_id, merge_player_id):
    ...
    for row in rows:
        ...
        if existing:
            ...
            conn.execute("DELETE FROM player_aliases WHERE player_alias_id = ?", (row["player_alias_id"],))
        else:
            conn.execute(
...
def merge_player(conn, keep_player_id, merge_player_id):
    ...
    conn.execute("UPDATE player_friend_codes SET player_id = ? WHERE player_id = ?", (keep_player_id, merge_player_id))
    conn.execute("UPDATE match_players SET player_id = ? WHERE player_id = ?", (keep_player_id, merge_player_id))
    conn.execute("UPDATE race_player_results SET player_id = ? WHERE player_id = ?", (keep_player_id, merge_player_id))
...
def main():
    parser = argparse.ArgumentParser(description="Merge reviewed player identities by friend-code map.")
    parser.add_argument("--db", type=Path, default=DEFAULT_DB_PATH, help="SQLite database path.")
    parser.add_argument("--identity-map", type=Path, default=IDENTITY_PATH, help="Player identity CSV path.")
