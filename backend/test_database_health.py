...
class DatabaseHealthTests(unittest.TestCase):
    ...
    def test_dismissed_catalog_warning_remains_visible_but_does_not_degrade_health(self):
        self.session.add_all([Track(canonical_name="Test Track"), Track(canonical_name="Test-Track")])
        self.session.commit()
        ...
        issue = next(item for item in report["issues"] if item["key"] == "duplicate-track:testtrack")
    ...
    def test_alias_collision_uses_actual_match_scope_and_canonical_names(self):
        ...
        self.session.commit()
        self.session.add_all([
            Division(division_id=1, season_id=1, division_code="d1", division_name="Division 1"),
            Division(division_id=2, season_id=2, division_code="d1", division_name="Division 1"),
        ])
        self.session.commit()
        self.session.add_all([
            TeamSeasonEntry(
        ...
        collision_issues = [
            ...
            if item["key"].startswith("player-alias-collision:")
        ]
        issue = next(item for item in collision_issues if item["key"] == "player-alias-collision:s3:d1:mii_name:holy")
...
class DatabaseHealthApiTests(unittest.TestCase):
    ...
    def test_health_route_forwards_archive_option(self):
        ...
        with patch.object(app_module.stats, "SessionLocal", return_value=nullcontext(object())), patch.object(
    ...
    def test_review_route_persists_dismissible_finding(self):
        ...
        with patch.object(app_module.stats, "SessionLocal", return_value=nullcontext(object())), patch.object(
            app_module.database_health_service, "build_database_health", return_value=report
        ), patch.object(app_module, "set_issue_review", return_value=review) as save_review:
            response = self.client.post("/api/database-health/reviews", json={
